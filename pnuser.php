 ...
 */
function quickcheck_user_view($args) {
...
function quickcheck_user_renderquiz($args) {
    ...
    $questions = pnModAPIFunc('quickcheck', 'user', 'getallquestions');
    usort($questions, sort_by_catid);
    //walk the data array and see where we need questions
    $notice = array();
    foreach($data as $cat_id => $number ) {
        $cat_questions = _fetch_cat_questions($questions, $cat_id);
    ...
    return _display_quiz($quiz_questions, $ret_url, "", $notice);

    return true;
}

function _fetch_cat_questions($in_questions, $in_cat_id){
    $ret_questions = array();
    $started = false;
 ...
 *
 */
function quickcheck_user_display($args) {
    ...
    foreach($exam['questions'] as $quest) {
        $questions[] = pnModAPIFunc('quickcheck', 'user', 'getquestion', array('id' => $quest));
    }
    return _display_quiz($questions, $return_url, $exam['name']);
}
 ...
 * @return the text of the quiz.
 */
function _display_quiz($questions, $return_url, $exam_name, $notice=null){
    ...
    $pnRender->assign('letters', $letters);
    $pnRender->assign('q_ids', $q_ids);
    $pnRender->assign('questions', $questions);
    $pnRender->assign('notice', $notice);
    $pnRender->assign('ret_url', $return_url);
    $pnRender->assign('exam_name', $exam_name);

    // Return the output that has been generated by this function
    return $pnRender->fetch('quickcheck_user_display.htm');
}
/**
 * gradequize
 *
...
function quickcheck_user_gradequiz($args) {
    ...
    foreach($q_ids as $q_id) {
        ...
        $question = pnModAPIFunc('quickcheck', 'user', 'getquestion', array('id' => $q_id));
