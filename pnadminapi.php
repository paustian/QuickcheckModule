 ...
 */
function quickcheck_adminapi_create($args)
{
 ...
 */
function quickcheck_adminapi_createquestion($args)
{
 ...
 */
function quickcheck_adminapi_delete($args)
{
    $id = $args['id'];
    // Argument check - make sure that all required arguments are present,
    // if not then set an appropriate error message and return
   ...
   $item = pnModAPIFunc('quickcheck', 'user', 'get', array('id' => $id));
...
function quickcheck_adminapi_deletequestion($args)
{
    // Security check
    // In this case we had to wait until we could obtain the item
    // name to complete the instance information so this is the first
 ...
 * @return   bool             true on success, false on failure
 */
function quickcheck_adminapi_update($args)
{
    // Argument check - make sure that all required arguments are present,
    // if not then set an appropriate error message and return
    ...
    if(!DBUtil::updateObject($args, 'quickcheck_exam')){
    	return LogUtil::registerError (__("Update of the exam failed."));
    }
 ...
 * @return   bool               true on success, false on failure
*/
function quickcheck_adminapi_updatequestion($args)
{
    // Argument check - make sure that all required arguments are present,
    // if not then set an appropriate error message and return
    ...
    if(!DBUtil::updateObject($args, 'quickcheck_quest')){
    	return LogUtil::registerError (__("Updating the question failed"));
    }
    // Let the calling process know that we have finished successfully
    return true;
}
...
function quickcheck_adminapi_import($args){
    ...
    foreach($matches[1] as $q_item){
        ...
        if(preg_match("|<qid>(.*?)</qid>|", $q_item, $q_id)){
            $q_data['id'] = $q_id[1];
            $item = pnModAPIFunc('quickcheck', 'user', 'getquestion', array('id' => $q_data['id']));
            $do_update = ($item != false);
        }
        if ($do_update) {
            
            if (!DBUtil::updateObject($q_data, 'quickcheck_quest', 'id')) {
                return LogUtil::registerError (__("insert of question failed"));
            }
        } else {
...
function quickcheck_adminapi_export($args){
    if (!SecurityUtil::checkPermission('quickcheck::', "::", ACCESS_ADMIN)) {
        LogUtil::registerAuthidError();
        return false;
    }
    ...
    if ($export_all == 'on') {
        ...
        foreach ($q_ids as $question_id) {
            $question = pnModAPIFunc('quickcheck', 'user', 'getquestion', array('id' => $question_id));
            $questions[] = $question;
        }
    }
